...
tests = testGroup "ControlHierarchy"
  [ testGroup "buildControlIndex"
    [ testCase "outer type block (tdWithin = Nothing) keys on (own name, \"this\")" $
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "w_main" "window" Nothing) []] }
            idx = buildControlIndex [sf]
        in Map.lookup ("w_main", "this") idx @?=
             Just (ControlDecl "w_main" "this" "window" Nothing Nothing)

    , testCase "child control keys on (parent literal name, control name)" $
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "dw_1" "datawindow" (Just "w_main")) []] }
            idx = buildControlIndex [sf]
        in Map.lookup ("w_main", "dw_1") idx @?=
    ...
    , testCase "backtick ancestor splits into cdAncestorType/cdOverridesName via D1" $
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "page1" "w_form_tab2`page1" (Just "tab1")) []] }
            idx = buildControlIndex [sf]
        in Map.lookup ("tab1", "page1") idx @?=
             Just (ControlDecl "tab1" "page1" "w_form_tab2" (Just "page1") Nothing)

    , testCase "literal dataobject binding is captured as cdDwBinding" $
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "dw_1" "datawindow" (Just "w_main")) (dataObjectBody "d_items")] }
            idx = buildControlIndex [sf]
        in (cdDwBinding <$> Map.lookup ("w_main", "dw_1") idx) @?= Just (Just "d_items")

    , testCase "control with no dataobject literal has cdDwBinding = Nothing" $
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "dw_1" "datawindow" (Just "w_main")) []] }
            idx = buildControlIndex [sf]
        in (cdDwBinding <$> Map.lookup ("w_main", "dw_1") idx) @?= Just Nothing

    , testCase "merges entries across multiple SrFiles into one workspace index" $
        let sfA = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "a" "window" Nothing) []] }
            sfB = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "b" "window" Nothing) []] }
            idx = buildControlIndex [sfA, sfB]
        in (Map.member ("a", "this") idx, Map.member ("b", "this") idx) @?= (True, True)
  ...
  , testGroup "resolveMemberChainType"
    ...
    , testCase "backtick override resolves through the ancestor's own declaration of the same-named control" $
        -- tab1 (in the descendant) overrides w_form_tab2's own tab1, whose
        -- real (non-overridden) type is "tab".
        let sf = emptyFile
              { srTypeBlocks =
    ...
    , testCase "chain falls back to resolved effective type when no literal-name declaration exists for the next hop" $
        ...
        -- CLASS uo_grid, never within the instance name "uo_epidom".
        let sf = emptyFile
              { srTypeBlocks =
    ...
    , testCase "ancestor-chain lookup finds a control declared on an ancestor object, not locally redeclared" $
        let sf = emptyFile
              { srTypeBlocks = [TypeBlock (TypeDecl "dw_1" "datawindow" (Just "w_ancestor")) []] }
            idx = buildControlIndex [sf]
    ...
    , testCase "cycle in override chain terminates rather than looping" $
        ...
        let sf = emptyFile { srTypeBlocks = [TypeBlock (TypeDecl "x" "a`x" (Just "a")) []] }
            idx = buildControlIndex [sf]
    ...
    , testCase "unresolvable hop midway through the chain returns Nothing, not a partial guess" $
        let sf = emptyFile
              { srTypeBlocks = [TypeBlock (TypeDecl "tab1" "tab" (Just "w_main")) []] }
            idx = buildControlIndex [sf]
