"""Unit tests for pb.pipeline.commands — corpus."""
...
def test_corpus_routes_to_build_runner(monkeypatch, tmp_path):
    ...
    from pb.pipeline.commands import corpus
    ...
    monkeypatch.setattr("pb.pipeline.commands.corpus.env.build.find_repo", fake_find_repo)
    monkeypatch.setattr("pb.pipeline.commands.corpus.env.build.build_runner", fake_build_runner)
    ...
    with patch("pb.pipeline.commands.corpus.subprocess.run") as mock_run:
...
def test_corpus_routes_to_find_binary(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("pb.pipeline.commands.corpus.env.build.find_repo", fake_find_repo)
    monkeypatch.setattr("pb.pipeline.commands.corpus.env.build.find_binary", fake_find_binary)
