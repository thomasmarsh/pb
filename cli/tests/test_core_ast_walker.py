...
def test_walk_calls_nested():
    node = {"tag": "BsIf", "contents": {
        "cond": {"tag": "ExCall", "callee": {"segments": [{"name": "f1"}]}},
        "then": [{"tag": "BsCall", "contents": {"tag": "ExCall",
                  "callee": {"segments": [{"name": "f2"}]}}}]
    }}
    calls = walk_calls(node)
    assert len(calls) == 2
...
def test_walk_bsraw_nested():
    node = {"tag": "BsIf", "contents": {"cond": {}, "then": [
        {"tag": "BsRaw", "contents": "INSERT INTO t"}
    ], "elseIfs": [], "else": None}}
