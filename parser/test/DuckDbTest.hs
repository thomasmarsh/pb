...
testAppendObjects = withWriteConn ":memory:" $ \conn -> do
  initSchema conn
  appendObjects conn
    [ ObjectRow "test.srf" "powerscript" "w_test" (Just "w_ancestor")
    , ObjectRow "other.sru" "powerscript" "u_util" Nothing
    ]
...
testAppendProcedures = withWriteConn ":memory:" $ \conn -> do
  ...
  appendProcedures conn
    [ ProcRow "test.srf" "w_test" "open"  "event"      1  10 cfgJs cpsJs
    , ProcRow "test.srf" "w_test" "close" "event"     11  20 cfgJs cpsJs
    ]
