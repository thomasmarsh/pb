...
// Αφαίρεση του αρχικού AND αν έχουμε δώσει κάποια ημ/νία
...
// Αφαίρεση του αρχικού OR αν έχουμε δώσει κάποια κράτηση
...
// Συνδυασμός ls_where_datefinal, ls_where_plirdate, ls_where_krat, ls_where_period
	if not isnull(ls_where_datefinal) and ls_where_datefinal <> "" then
...
public function boolean if_check4required ();// Έλεγχος αν έχουν εισαχθεί όλα τα κριτήρια
...
// Έλεγχος αν έχουμε επιλέξει τουλάχιστον μία κράτηση
...
public function boolean if_krat_selected ();// Έλεγχος αν επιλέχθηκε τουλάχιστον μία κράτηση
...
// Αφαίρεση του αρχικού AND (5 χαρακτήρες με τα 2 κενά)
// και προσθήκη παρενθέσεων
	ls_where = right(ls_where, Len(ls_where) - 5)
	ls_where = "(" + ls_where + ")"


// Αν ib_includewhere = true, προσθήκη " WHERE "
// Αν είναι false προσθήκη " AND "
// (Το " AND " αφαιρέθηκε προηγουμένως για να μπουν παρενθέσεις)
