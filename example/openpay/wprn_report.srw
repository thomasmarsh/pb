...
// Δημιουργία datastore για να πάρουμε όλες τις στήλες
// της αναφοράς il_kodreport
	datastore	lds_cols
	string		ls_sql
	long			ll_ncols
...
// Προσθήκη header, data, sum
	...
	// Παίρνουμε το τις τρέχουσες τιμές position
	// για να υπολογίσουμε από που αρχίζουν τα cols
	...
	// Παίρνουμε την οριζόντια απόσταση από ini
		integer	li_space
		
		li_space = ProfileInt(gs_ini_file, "REPORT", "space",0)
	
	// Yπολογίζουμε το 'x' από όπου αρχίζουν τα cols
		li_x = li_data_x + li_data_width
		
	
	for i = 1 to ll_ncols
		
		// Παίρνουμε τον κωδικό της επόμενης στήλης
			li_kodcol = lds_cols.object.kodcol[i]
	
		// Παίρνουμε τα δεδομένα της στήλης
			fn_loadcol(is_kodreport, li_kodcol, lsc_col)
		
		// Προσθέτουμε την οριζόντια απόσταση
			li_x = li_x + li_space
...
public subroutine if_addcol (ref datawindow adw, integer ai_kodcol, ref integer ai_x, integer ai_header_y, integer ai_data_y);// Προσθήκη στήλης στο adw
// Ο κωδικός στήλης βρίσκεται στο ads_cols.object.kodcol[row]
// 
/*
// Παίρνουμε τα δεδομένα της στήλης
	sc_col		lsc_col
...
// Προσθήκη summary (computed field)
	adw.Modify( &
...
public subroutine of_open ();// Παίρνουμε τις μεταβλητές
	il_kodfinal = message.doubleparm
	is_kodreport = gstring

// Γράφουμε τα στοιχεία της μισθοδοσίας στο dw
	date		ldt_datefinal
	string	ls_title
...
public subroutine if_add_header (ref datawindow adw, sc_col asc_col, integer ai_x, integer ai_y, integer ai_height);// Προσθήκη header
...
public subroutine if_add_data (ref datawindow adw, sc_col asc_col, integer ai_x, integer ai_y, integer ai_height);// Προσθήκη data
...
public subroutine if_add_sum (ref datawindow adw, sc_col asc_col, integer ai_x, integer ai_y, integer ai_height);// Προσθήκη summary (computed field)
...
public subroutine if_modify_existed (ref datawindow adw, integer ai_x);// Μετακίνηση σταθερών στοιχείων στο σημείων 'ai_x'
// (το δεξιότερο σημείο)
