...
protected function boolean if_checkusername (string as_username);// Έλεγχος αν υπάρχει το as_username στη βάση
...
protected function boolean if_checkinput (string as_username, string as_password);// Έλεγχος αν δώσαμε username και password
...
protected function boolean if_checkuseractive (string as_username);// Αν το username υπάρχει έλεγχος αν είναι active
	boolean	lb_isactive
...
protected function boolean if_checkpassword (string as_username, string as_password);// Έλεγχος του κωδικού πρόσβασης στη βάση
...
protected function boolean if_checkapp (string as_username, string as_kodapp);// Ελέγχουμε αν ο χρήστης έχει δικαίωμα πρόσβασης
// στην τρέχουσα εφαρμογή (gs_kodapp)
	long		ll_koduser
	integer	li_flag
	
// Ο χρήστης με koduser = -1 (master)
// έχει πάντα δικαίωμα πρόσβασης	
	setnull(ll_koduser)
	select koduser into :ll_koduser from usrusers
	where	 username = :as_username;
	fn_sqlerror()
...
// Για τους άλλους χρήστες έλεγχος του πίνακα usrUserPerm
	integer	li_enable
	...
	if isnull(li_enable) or li_enable = 0 then
		MessageBox(trn(102), tr("Αυτός ο χρήστης δεν έχει δικαίωμα πρόσβασης σ' αυτή την εφαρμογή"), Exclamation!)
		sle_user.setfocus()
		return false
	else
		return true
...
event clicked;// Έξοδος από την εφαρμογή
	HALT CLOSE
end event
...
event clicked;// Παίρνουμε τα στοιχεία που πληκτρολογήσαμε
	string	ls_username, &
				ls_password
...
// Πρόσβαση επιτρέπεται. Αποθήκευση στις καθολικές
...
integer width = 590
integer height = 640
string picturename = "D:\projects\share\res\ΥΠΟΣΤΗΡΙΞΗ.WMF"
boolean border = true
