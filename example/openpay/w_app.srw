...
event ie_sizebar();// Προσαρμογή του tv

int cxClient, cyClient		// client area
int xClient, yClient			// Απόσταση από τα όρια του παραθύρου
int cxMDI, cyMDI				// mdi_1 object's dimensions

// Παίρνουμε τις νέες διαστάσεις της Client Area και την απόσταση της από τις άκρες
	cxClient = WorkspaceWidth()
	cyClient = WorkspaceHeight() - mdi_1.microhelpheight	// microhelp included in WorkspaceHeight()
	xClient = WorkspaceX()
	yClient = WorkspaceY()
	
// Αλλάζουμε το ύψος του tv και το ξανατοποθετούμε
	tv.Height = cyClient
	tv.move(xClient, yClient)
	
// Ξανακαθορίζουμε τις διαστάσεις του αντικειμένου mdi_1
...
event doubleclicked;// Παίρνουμε το data του handle
