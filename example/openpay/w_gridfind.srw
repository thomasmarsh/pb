...
public subroutine if_find ();// Αναζήτηση με βάση τις επιλογές
	string	ls_findwhat
	integer	li_lookat
	integer	li_match
	
// Παίρνουμε τα δεδομένα από dw_find
	dw_find.AcceptText()
	ls_findwhat = dw_find.object.findwhat[1]
	li_lookat = dw_find.object.lookat[1]
	li_match = dw_find.object.match[1]
	
// Αν δεν δώσαμε κείμενο αναζήτησης επιστρέφουμε
...
// Αναζήτηση στο τρέχον πεδίο ή σε όλο τον πίνακα
	long		ll_rowfound
	
	choose case li_lookat	// Ανάλογα με Διερεύνηση σε
			
		case 1		// Τρέχον πεδίο
			
			Choose case li_match	// Ανάλογα με ταίριασμα
					
				case 1	// Ολόκληρο το πεδίο
					ll_rowfound = idw.Find("#" + string(ll_curcol) + "='" + &
												  ls_findwhat + "'", il_nextrow, ll_nrows)
					
				case 2	// Οποιοδήποτε τμήμα του πεδίου
					
				case 3	// Αρχή του πεδίου
