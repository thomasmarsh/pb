...
event cb_ok::clicked;// Αν δεν υπάρχει γραμμή επιστρέφουμε 
	long	ll_row
	ll_row = dw_main.getrow()
...
// Παίρουμε τον κωδικό του φίλτρου
	long	ll_kodfilter
	ll_kodfilter = dw_main.object.kodfilter[ll_row]

// Καθαρίζουμε τα δεδομένα του iw_parent.idw_filter
	long	ll_rows, i
...
// Δημιουργία datastore με τα δεδομένα του dw_afxfilterd_list
// για το επιλεγμένο φίλτρο (ll_kodfilter)
...
// Φορτώνουμε τα δεδομένα από το lds_filterd στο iw_filter.idw_filter
	for i = 1 to ll_rows
...
event clicked;// Διαγραφή του επιλεγμένου φίλτρου
// Γίνεται cascade delete στο afxFilterD
