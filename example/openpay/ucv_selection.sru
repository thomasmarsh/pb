...
public subroutine fn_initdw (string as_dwobject);// Θέτει το dw object στα δύο dw's 
...
public function datawindow fn_getselecteddw ();// Επιστρέφει το dw_selected
...
public subroutine of_target2source (ref datawindow source_dw, long source_row, ref datawindow target_dw, long target_row);// Μεταφορά δεδομένων από dw_target πίσω στο dw_source
end subroutine

public function boolean of_match (ref datawindow source_dw, long source_row, ref datawindow target_dw, long target_row);// Επιστρέφει true αν οι δύο εγγραφές (arguments)
// είναι ίδιες.
// Περιέχει τον έλεγχο για να είναι ίδιες οι εγγραφές
// (ελέγχει τις τιμές των κλειδιών)
...
public subroutine if_setbuttons ();// Καθορίζει την κατάσταση των πλήκτρων
// ανάλογα με τις διαθέσιμες εγγραφές


// Βρίσκουμε τον αριθμό των εγγραφών στα δύο dw's
	long	ll_source_rows, ll_target_rows
	ll_source_rows = dw_source.RowCount()
	ll_target_rows  = dw_target.RowCount()
	
// Κουμπιά μεταφοράς στο dw_target
...
// Κουμπιά μεταφοράς πίσω στο dw_source
...
event clicked;// Μεταφορά όλων από το dw_target στο dw_source
	
// Set redraw false for both dw's
	dw_source.SetRedraw(false)
	dw_target.SetRedraw(false)

// Προσθέτουμε όλες τις εγγραφές του dw_source στο dw_target
	long	ll_target_rows, &
			i
...
event clicked;// Διαγράφει την επιλογή στο dw_target και την προσθέτει στο dw_source
	
// Set redraw false for both dw's
	dw_source.SetRedraw(false)
	dw_target.SetRedraw(false)

// Βρίσκουμε την επιλεγμένη εγγραφή του dw_target
	long	ll_target_row
	ll_target_row = dw_target.GetRow()
	if ll_target_row = 0 then return

// Προσθέτουμε μία εγγραφή στο dw_source
	long	ll_source_row
...
// Διαγραφή της επιλεγμένης γραμμής στο dw_target
// και επιλογή της προηγούμενης
...
// Επιλογή της νέας γραμμής στο dw_source
...
// Καθορίζουμε την κατάσταση των πλήκτρων
	if_setbuttons()
...
event clicked;// Μεταφορά όλων από το dw_source στο dw_target
...
event clicked;// Διαγράφει την επιλογή στο dw_source και την προσθέτει στο dw_target
	
// Set redraw false for both dw's
	dw_source.SetRedraw(false)
	dw_target.SetRedraw(false)

// Βρίσκουμε την επιλεγμένη εγγραφή του dw_source
	long	ll_source_row
	ll_source_row = dw_source.GetRow()
	if ll_source_row = 0 then return

// Προσθέτουμε μία εγγραφή στο dw_target 
	long	ll_target_row
...
// Διαγραφή της επιλεγμένης γραμμής στο dw_source
// και επιλογή της προηγούμενης
...
// Επιλογή της νέας γραμμής στο dw_target
	dw_target.Setrow(ll_target_row)
...
event doubleclicked;// Προσομοίωση cb_remove_one
	if row > 0 then cb_remove_one.TriggerEvent(clicked!)
end event

event retrieverow;
// Αν η νέα εγγραφή υπάρχει στο dw_source (of_match)
// τότε την διαγράφουμε από το dw_source
// Υποτίθεται ότι το retrieve στο dw_source έχει γίνει (στο ie_retrieve)

// Παίρνουμε τον αριθμό των εγγραφών του dw_source
	long	ll_source_rows
	ll_source_rows = dw_source.rowcount()
	
// Έλεγχος της νέας retrieved εγγραφές με όλες του dw_source
// και διαγραφή αν υπάρχει. Μετά την διαγραφή δεν συνεχίζουμε
// με την εξέταση των επομένων (μία μόνο εγγραφή υπάρχει)
	long	i
	for i = 1 to ll_source_rows
...
event doubleclicked;// Προσομοίωση cb_add_one
