// core/job-poll.ts — Generic background-job submit/poll state machine (Plan 159).
//
// Domain-agnostic: no diagram knowledge. Any consumer with a job that a
// server executes asynchronously and reports back via a job id embeds this
// reducer via its own keyed dictionary (see app/reducer.ts's `inlineDiagram`
// handling for the established keyed-dictionary pattern this follows).
...
export function jobPollReduce<T>(
  ...
  case "polled": {
    ...
    if (draft.attempt >= JOB_POLL_MAX_ATTEMPTS) {
      draft.status = "error";
      draft.error = "Timed out waiting for diagram render";
      return null;
    }
