...
import { diagramsReducer, type DiagramsEnv } from "../../src/features/diagrams/reducer.js";
...
describe("diagrams reducer", () => {
  describe("diagrams/select", () => {
    it("updates active kind and clears svg", () => {
      const init = { ...diagramsReducer.initialState(), svg: "<svg>old</svg>" };
  ...
  describe("diagrams/params", () => {
    it("merges params into state", () => {
      const ts = createTestStore(diagramsReducer, mockEnv, diagramsReducer.initialState());
  ...
  describe("diagrams/generate", () => {
    ...
    it("populates svg via effect", () => {
      ...
      const ts = createTestStore(diagramsReducer, env, diagramsReducer.initialState());
      ts.send({ type: "generate" }, (s) => {
        s.loading = true;
      });
